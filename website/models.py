from website import db, login_manager, bcrypt  # Importing necessary modules
from flask_login import UserMixin
from sqlalchemy.ext.hybrid import hybrid_property
from flask_validator import ValidateEmail, ValidateCountry
from website.custom_validators import ValidatePhoneNumber
...
class User(db.Model, UserMixin):
    ...
    passport = db.relationship('Passport',  # Relationship with Passport model
                               back_populates="user",
                               uselist=False,
                               lazy=False)
    employment = db.relationship('Employment',  # Relationship with Employment model
                                 back_populates="user",
                                 uselist=False,
                                 lazy=True)
    bookings = db.relationship('Booking',  # Relationship with Booking model
                               back_populates="user",
                               lazy=True)
    ...
    def __declare_last__(cls):
        ...
        ValidateEmail(User.email,  # Validate email
                      allow_smtputf8=True,
                      check_deliverability=True,
                      throw_exception=True,
                      message="The e-mail is invalid.")
        ValidatePhoneNumber(User.phone_number,  # Validate phone number
                            allow_null=True,
                            throw_exception=True,
...
class Company(db.Model):
    ...
    employees = db.relationship('Employment',  # Relationship with Employment model
    ...
    @classmethod
    def __declare_last__(cls):
        ValidateEmail(Company.email,  # Validate email
                      allow_smtputf8=True,
                      check_deliverability=True,
                      throw_exception=True,
                      message="The e-mail is invalid.")
        ValidatePhoneNumber(Company.phone_number,  # Validate phone number
                            allow_null=True,
                            throw_exception=True,
                            message="Phone number is invalid.")
...
class Country(db.Model):
    ...
    cities = db.relationship('City',  # Relationship with City model
                             back_populates="country",
                             lazy=True)
    passports = db.relationship('Passport',  # Relationship with Passport model
                                back_populates="country",
                                lazy=True)
    ...
    @classmethod
    def __declare_last__(cls):
        ValidateCountry(Country.name,  # Validate country name
                        allow_null=False,
                        throw_exception=True,
...
class City(db.Model):
    ...
    country_id = db.Column(db.Integer,
                           ...
                           nullable=False)
    country = db.relationship('Country',  # Relationship with Country model
                              back_populates="cities",
                              lazy=True)
    arrivals = db.relationship('Flight',  # Relationship with Flight model (arrivals)
                               foreign_keys="Flight.arrival_city_id",
                               back_populates="arrival_city",
                               lazy=True)
    departures = db.relationship('Flight',  # Relationship with Flight model (departures)
...
class Flight(db.Model):
    ...
    arrival_city_id = db.Column(db.Integer,
                                ...
                                nullable=False)
    arrival_city = db.relationship('City',  # Relationship with City model (arrival city)
    ...
    departure_city_id = db.Column(db.Integer,
                                  db.ForeignKey('city.id'),
                                  nullable=False)
    departure_city = db.relationship('City',  # Relationship with City model (departure city)
...
class Employment(db.Model):
    ...
    company_id = db.Column(db.Integer,
                           ...
                           nullable=False)
    company = db.relationship('Company',  # Relationship with Company model
...
class Booking(db.Model):
    ...
    flight_id = db.Column(db.Integer,
                          ...
                          nullable=False)
    flight = db.relationship('Flight',  # Relationship with Flight model
    ...
    def __init__(self, flight,
        ...
        self.employment = issuing_employment
        self.date_issued = date_issued
        self.cancellation_fee = cancellation_fee

    def __repr__(self):
        return (f"Booking('{self.user}', on '{self.flight}'). "
                f"Issued on: {self.date_issued}")
